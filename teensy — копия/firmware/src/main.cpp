...
void moveBase()
{
    ...
    pwmR = (g_req_linear_vel_x - g_req_angular_vel_z/2)*200;
    pwmL = (g_req_linear_vel_x + g_req_angular_vel_z/2)*200;
    pwmR = constrain(pwmR, -200, 200);
    pwmL = constrain(pwmL, -200, 200);

    // Serial.print("pwmR is: ");
    // Serial.println(pwmR);

    // Serial.print("pwmL is: ");
    // Serial.println(pwmL);

    if(pwmR>=0){
        analogWrite(MOTOR_A_BCW, 0);
        analogWrite(MOTOR_A_FWD, pwmR);
    }
    else{
        analogWrite(MOTOR_A_FWD, 0);
        analogWrite(MOTOR_A_BCW, -pwmR);
    }
    if(pwmL>=0){
        analogWrite(MOTOR_B_BCW, 0);
        analogWrite(MOTOR_B_FWD, pwmL);
    }
    else{
        analogWrite(MOTOR_B_FWD, 0);
        analogWrite(MOTOR_B_BCW, -pwmL);
    }
